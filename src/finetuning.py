...
def inject_lora_into_model(model, r=4, alpha=32, device='cpu'):
    ...
    for child_name, child_module in model.named_children():
        ...
        parent_cls = model.__class__.__name__.lower()
        print(parent_cls)
        if parent_cls in {"t5attention"} and isinstance(child_module, nn.Linear) and (child_name.lower() in {"k","v","q","o"}) and not isinstance(child_module, LoRA):
